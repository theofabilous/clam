...
#define GET_NTH_IMPL(n, ...)    \
    ...
    (\
        ...
        OBSTRUCT(GET_NTH_IMPL_INDIRECT) ()         \
        (                                           \
            DEC(n), WO_FIRST(__VA_ARGS__)               \
        )\
    )
...
#define CHECK_SIZE_EQ_1(...) COMPL(BOOL(CAT(GET_LAST_EQ_, PP_NARG(__VA_ARGS__))))

#define GET_LAST_IMPL(...) \
    IF(CHECK_SIZE_EQ_1(__VA_ARGS__))\
    (\
        GET_FIRST(__VA_ARGS__),\
        OBSTRUCT(GET_LAST_IMPL_INDIRECT) ()         \
        (                                           \
            WO_FIRST(__VA_ARGS__)                   \
        )\
    )
...
#define ZIP_IMPL(l1, l2) \
    ...
    (\
        (GET_FIRST l1, GET_FIRST l2),\
        (GET_FIRST l1, GET_FIRST l2),\
        OBSTRUCT(ZIP_IMPL_INDIRECT) ()         \
        (                                           \
            (WO_FIRST l1), (WO_FIRST l2)                   \
        )\
    )
